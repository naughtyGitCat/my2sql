...
func (this *ConfCmd) ParseCmdOptions() {
	...
	if this.Mode == "file" {
		...
		// use local binlog file for preference
		if this.LocalBinFile != "" {
			this.GivenBinlogFile = this.LocalBinFile
		}
		if this.GivenBinlogFile == "" {
			log.Fatalf("missing binlog file.  -start-file or -local-bin-file must be specify when -mode=file ")
		}
		if !toolkits.IsFile(this.GivenBinlogFile) {
			log.Fatalf("%s does not exists nor a file\n", this.GivenBinlogFile)
		} else {
